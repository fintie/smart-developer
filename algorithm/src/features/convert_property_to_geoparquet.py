...
CHUNKS_DIR = ROOT / "data" / "raw" / "nsw_property" / "chunks"
OUTPUT_DIR = ROOT / "data" / "processed" / "nsw_property"
OUTPUT_PATH = OUTPUT_DIR / "property.parquet"
...
def main() -> None:
    ...
    for i, chunk_file in enumerate(chunk_files, start=1):
        try:
            ...
            if len(obj_cols) > 0:
                gdf[obj_cols] = gdf[obj_cols].astype("string")
    ...
    if not frames:
        raise RuntimeError("No property chunk files could be read.")

    print("Concatenating property frames...")
    ...
    if failed:
        ...
        print("\nAll property chunks read successfully.")
