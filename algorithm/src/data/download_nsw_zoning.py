...
RAW_DIR = ROOT / "data" / "raw" / "nsw_zoning"
...
def fetch_chunk(object_ids: list[int], chunk_idx: int, total_chunks: int) -> Path:
    ...
    out_path = CHUNKS_DIR / f"land_zoning_{chunk_idx:04d}_of_{total_chunks:04d}.geojson"
...
def main() -> None:
    ...
    for idx, chunk_ids in enumerate(chunks, start=1):
        out_path = CHUNKS_DIR / f"land_zoning_{idx:04d}_of_{total_chunks:04d}.geojson"
        if out_path.exists():
