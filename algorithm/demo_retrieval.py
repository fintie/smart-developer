...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Terminal demo for Smart Developer retrieval."
    )
    parser.add_argument(
        "--experiment",
        default="two_tower_v1",
        help="Retrieval experiment name from algorithm/configs/model.yaml",
    )
    parser.add_argument(
        "--strategy",
        help="Strategy name, e.g. low_rise_apartment",
    )
    parser.add_argument(
        "--query-text",
        help="Free-text intent query",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Retrieval similarity weight for fusion mode",
    )
    parser.add_argument(
        "--beta",
        type=float,
        default=0.5,
        help="Heuristic score weight for fusion mode",
    )
    parser.add_argument("--with-explanations", action="store_true")
    parser.add_argument(
        "--no-dcn-reranker",
        action="store_true",
        help="Disable DCN reranker and use fusion-only reranking",
    )
    parser.add_argument(
        "--dcn-experiment",
        default="dcn_reranker_v1",
